...
\section*{Localisation}
{
    ...
    \subsection*{Indoor Location}
    {
        ...
        {
            More general than just location, but intuitive for location.

            Have a floorplan, generate particles randomly in the environment with initial directions/velocities. As the device moves, move the particles a corresponding amount \textbf{+ noise} and remove particles which perform some impossible movement (through walls).

            Resample particles to ensure we don't run out, with clusters of particles being more likely to spawn more.

            Two stages:
            \begin{itemize}
            \item Localisation uses lots of particles to work out where the user is from an unknown initial state. 
            \item Tracking uses fewer particles once we've found the location to improve efficiency.
            \end{itemize}
        }
    }
}
